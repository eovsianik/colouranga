...
def read_image_as_np_array(image_path: str):
...
def visualise_single_image_prediction(image_as_np_array, predictions, filename):
    ...
    figure.canvas.draw()
    image = np.array(figure.canvas.renderer._renderer)  # type: ignore
    plt.close()
    return image
...
def get_distance(rectA, rectB):
    return box(rectA[0], rectA[1], rectA[2], rectA[3]).distance(
        box(rectB[0], rectB[1], rectB[2], rectB[3])
    )
...
def use_cuts_to_determine_edge_from_a_to_b(a, b, rects):
    ...
    while True:
        xmin, ymin, xmax, ymax = (
            min(rects[a][0], rects[b][0]),
            min(rects[a][1], rects[b][1]),
            max(rects[a][2], rects[b][2]),
            max(rects[a][3], rects[b][3]),
        )
        rect_index = [
            i for i in range(len(rects)) if intersects(rects[i], [xmin, ymin, xmax, ymax])
        ]
...
def sort_text_boxes_in_reading_order(text_bboxes, sorted_panel_bboxes):
    ...
    for group in grouped_indices:
        ...
        indices_of_texts[group[0] : group[-1] + 1] = [
            subset_of_text_indices[i] for i in sorted_subset_indices
        ]
