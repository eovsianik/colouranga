...
class MyMagiModel(PreTrainedModel):
    ...
    # def _get_predicted_bboxes_and_classes_new(
    #     self,
    ...
    def get_crops_and_embeddings(self, images: list[NDArray]):
        ...
        batch_bboxes, batch_character_indices, batch_scores = self.get_batch_bboxes_and_batch_character_indices(
    ...
    def predict_detections_and_associations(
        ...
        def get_character_character_matching_scores(batch_character_indices, batch_bboxes):
            ...
            #! НАДО ЭТУ ЧАСТЬ ВКЛЮЧИТЬ В ИНОЕ МЕСТО
            #! ЭТИ ДВЕ СТРОКИ ПЕРЕНЕСЕНЫ
            crop_bboxes = [
                batch_bboxes[i][batch_character_indices[i]]
                for i in range(len(batch_character_indices))
            ]

            #! ТУТ ПОСТРАНИЧНЫЕ ЭМБЕДИНГИ
            #! НАДО ЭТУ ЧАСТЬ ВКЛЮЧИТЬ В ИНОЕ МЕСТО
...
"""    def unpacking_characters_information(тут должна быть эта библиотека):
            библиотека распаковывается и получаем из сивиси файла словарь или отдельные переменные
            return 0"""


"""    def recording_information_to_pack(либо файл сивиси, либо библиотека,необходимые переменные для affinity matrix):
        return 1"""
